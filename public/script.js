...
equiposGlobal = (await res.json()).map(e => ({
  ...e,
  necesitaMantenimiento: e.necesitaMantenimiento
    ? String(e.necesitaMantenimiento).trim() 
    : "No"
}));
...
console.log("ID equipo desde URL:", idEquipo);

if (idEquipo) {
  const equipoQR = equiposFiltrados.find(e => e.id == idEquipo); // usar == por string/number
  if (equipoQR) {
    equiposFiltrados = [equipoQR];
  }
}

// Cargar oficinas si aplica
if (filterOffice) cargarOficinas();

// Finalmente, renderizamos todo **una sola vez**
renderTablaPaginada();
...
function aplicarFiltros() {
  ...
  if (searchInput && searchInput.value) {
    filtrados = filtrados.filter(e =>
      (e.codigoTorre || '').toLowerCase().includes(searchInput.value.atoLowerCase()) ||
